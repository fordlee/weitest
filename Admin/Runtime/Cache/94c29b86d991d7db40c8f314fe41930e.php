	...
	<div class="page-content">
		<div class="row">
			<div class="col-xs-12">
				...
				<form action="<?php echo U('answereditsave');?>" method="post" enctype="multipart/form-data" class="form-horizontal" >
					<input type="hidden" name="qdid" value="<?php echo ($qdItem['qdid']); ?>">
					<div class="form-group">
						<label class="col-sm-2 control-label no-padding-right" for="languageset">语言：</label>
						<div class="col-sm-2">
							<select id="languageset" name="languageset">
								<option value="zh" <?php if($qdItem['language'] == zh)echo selected ?>>
									中文
								</option>
								<option value="en" <?php if($qdItem['language'] == en)echo selected ?>>
									英语
								</option>
								<option value="pt" <?php if($qdItem['language'] ==pt)echo selected ?>>
									葡语
								</option>
							</select>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="content" placeholder="你在前世是怎么死去的？" class="col-sm-6" value="<?php echo ($qdItem['content']); ?>" />
						</div>
					</div>
					<?php if(is_array($aItem)): $i = 0; $__LIST__ = $aItem;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="form-group">
						<label class="col-sm-2 control-label no-padding-right">答案：</label>
						<div class="col-sm-10">
							<input type="hidden" name="aid[]" value="<?php echo ($v['id']); ?>">
							<input type="text" name="option[]" placeholder="xx死于xxxx年：在龙的战斗中战死" class="col-sm-6" value="<?php echo ($v['optionresult']); ?>"/>
							<div class="col-sm-2">
							...
							<?php if($v['answerpic'] != NULL): ?><img src="<?php echo ($v['answerpic']); ?>" /><?php endif; ?>
						</div>
