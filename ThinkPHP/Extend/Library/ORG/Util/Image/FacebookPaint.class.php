...
class FacebookPaint{
	...
	//注入生成gif图
	public function injectGif($set,$info){
       	...
       	foreach ($frames as $k1 => $v1) {
	        ...
	        foreach ($v1 as $k => $v) {
		        ...
		        if($v['type']=="text"){
		            ...
		            $im = $this -> injectText($im,$v['attribute'],$content);
		        }elseif($v['type']=="image"){
		            $im = $this -> injectImage($im,$v['attribute'],$content);
		        }
			}
		...
		$gif = new GIFEncoder($imagedata, $framed, $loop, 0, $r, $g, $b, "bin");
